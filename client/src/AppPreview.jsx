...
function Login({ onLogin }) {
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center",
      ...
      {/* Fond dégradé léger — identique WinWin V2 light */}
      <div style={{ position:"fixed", inset:0,
        background:"radial-gradient(ellipse 70% 50% at 15% 40%, rgba(49,118,166,.07) 0%, transparent 55%), radial-gradient(ellipse 50% 40% at 85% 20%, rgba(179,45,38,.04) 0%, transparent 50%), #F3F5F9" }}/>

      {/* Grille subtile */}
      <div style={{ position:"fixed", inset:0,
        backgroundImage:"linear-gradient(rgba(49,118,166,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(49,118,166,.04) 1px,transparent 1px)",
        backgroundSize:"48px 48px",
        maskImage:"radial-gradient(ellipse 80% 70% at center, black 20%, transparent 75%)" }}/>

      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:400, padding: w < 480 ? "16px" : "24px" }}>

        {/* Logo centré — version light */}
        <div style={{ display:"flex", justifyContent:"center", marginBottom:28 }}>
          <SwissRHLogo height={w < 420 ? 36 : 44}/>
        </div>

        {/* Card blanche — style WinWin V2 */}
        <div style={{ background:"#fff", border:"1px solid var(--border-default)",
          borderRadius:"var(--radius-lg)", padding: w < 480 ? "24px 20px" : "32px 28px",
          boxShadow:"0 8px 40px rgba(49,118,166,.10), 0 2px 8px rgba(0,0,0,.04)" }}>

          <div style={{ marginBottom:24 }}>
            <div style={{ fontWeight:800, fontSize:20, color:"var(--text-primary)", letterSpacing:"-.02em" }}>
              Connexion
            </div>
            <div style={{ fontSize:12, color:"var(--text-muted)", marginTop:4 }}>
              Accès sécurisé à votre espace RH · swissrh.ch
            </div>
          </div>

          {[["Email","email",email,setEmail],["Mot de passe","password",pass,setPass]].map(([lbl, type, val, set]) => (
            <div key={lbl} style={{ marginBottom:14 }}>
              <label style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".06em",
                color:"var(--text-muted)", display:"block", marginBottom:6 }}>{lbl}</label>
          ...
          <button className="btn btn-p" onClick={go} disabled={load}
            style={{ width:"100%", justifyContent:"center", padding:"11px", fontSize:13, fontWeight:700, marginTop:8 }}>
            {load ? <><Spinner/>&nbsp;Connexion en cours…</> : "→ Se connecter"}
          </button>

          <div style={{ marginTop:20, padding:"12px 14px", background:"var(--bg-void)",
            borderRadius:8, display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:16 }}>🔒</span>
            <div style={{ fontSize:10, color:"var(--text-muted)", lineHeight:1.5 }}>
              Connexion chiffrée TLS · Hébergé en Suisse<br/>
              <span style={{ color:"var(--accent-cyan)", fontWeight:600 }}>swissrh.ch</span> · Groupe NEO SA
            </div>
          </div>
        </div>
      </div>
